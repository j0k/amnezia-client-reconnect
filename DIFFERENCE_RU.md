...
- **«VPN proxy enabled (адрес)»** — когда включён прокси через VPN; в адресе показан LAN-IP
  (то, что вводить на другом ПК), и «· not running», если helper не поднят;
...
- **Запуск браузера через этот прокси** (у direct) — три кнопки: **Chrome / Edge** и
  **Yandex Browser** (Chromium: `--proxy-server=…` + отдельный `--user-data-dir`, чтобы окно не
  ...
  У каждого браузера (**Chrome, Edge, Firefox, Yandex Browser**) своя строка: кнопка *Launch*, кнопка
  *Copy* и **готовая командная строка (синтаксис PowerShell)**, которую выполняет кнопка — текст
  выделяется (работает Ctrl+C), строится из найденного пути к браузеру и текущих типа/порта прокси;
  неустановленные браузеры помечены *not found*.
...
- Windows/Linux: штатный глиф трея — белый на прозрачном и исчезает на светлой панели задач. Форк
  рисует его на **цветном круге, показывающем состояние** — зелёный = подключено, серый =
  отключено, красный = ошибка (`systemTrayNotificationHandler.cpp`, `setTrayIcon`).
- Добавлен **tooltip** с состоянием и повторный `show()` (переживает перезапуск Explorer/панели задач).
...
запускается (например, какие exe добавить в split-tunnel). Весь просматриваемый снимок можно вынести
из приложения: **Open whole list in editor** (выровненная текстовая таблица в
`%APPDATA%/AmneziaVPN/log/processes-<время>.txt`), **Copy list** (буфер обмена) и **Export .csv**
(разделитель `;`, UTF-8 с BOM + CRLF — Excel открывает сразу).
