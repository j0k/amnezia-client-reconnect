...
>   Практический пример: при включённом VPN прокси *мимо* VPN открывает сайты, закрытые *для иностранных IP* — например **matchtv.ru**, если ты в РФ, — потому что они видят твой реальный IP. И просто приятно: в окошке одного браузера один IP, в окошке другого — другой. 😊
...
> 📚 Доки: 🔁 Настройка Auto-reconnect и какие хосты пинговать — [RU](./docs/reconnect/SETUP_RU.md) / [EN](./docs/reconnect/SETUP.md) · 🔀 Архитектура локальных прокси (код + диаграммы) — [RU](./docs/proxy/ARCHITECTURE_RU.md) / [EN](./docs/proxy/ARCHITECTURE.md) · 📜 [История](./HISTORY.md)
>
